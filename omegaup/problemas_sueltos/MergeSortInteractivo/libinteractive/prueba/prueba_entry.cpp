	...
	while (readfull(&__in, &__msgid, sizeof(__msgid), 0)) {
		...
		switch (__msgid) {
			case 0x13857295u: {
				// Main -> prueba.mezclar
				uint32_t __cookie;
				int mezclar_izquierda;
				readfull(&__in, &mezclar_izquierda, sizeof(int), 1);
				int mezclar_derecha;
				readfull(&__in, &mezclar_derecha, sizeof(int), 1);
				readfull(&__in, &__cookie, sizeof(__cookie), 1);

				mezclar(mezclar_izquierda, mezclar_derecha);
...
void combinar(int izquierda, int mitad, int derecha) {
	...
	writefull(&__out, &__msgid, sizeof(__msgid));
	writefull(&__out, &izquierda, sizeof(int));
	writefull(&__out, &mitad, sizeof(int));
	writefull(&__out, &derecha, sizeof(int));
	writefull(&__out, &__cookie, sizeof(__cookie));
	writeflush(&__out);
...
int main(int argc, char* argv[]) {
	...
	openstream(&__in, "./prueba_in", O_RDONLY);
	openstream(&__out, "./prueba_out", O_WRONLY);
