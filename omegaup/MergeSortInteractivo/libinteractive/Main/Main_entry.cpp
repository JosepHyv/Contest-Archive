...
static struct __stream __prueba_in, __prueba_out;

void __NOINLINE __libinteractive_init() {
	openstream(&__prueba_in, "./prueba_in", O_WRONLY);
	openstream(&__prueba_out, "./prueba_out", O_RDONLY);
}
...
static void __message_loop_prueba(int __current_function, int __noreturn) {
	uint32_t __msgid;
	while (readfull(&__prueba_out, &__msgid, sizeof(__msgid), 0)) {
		if (__msgid == __current_function) return;
		switch (__msgid) {
			case 0x71651df7u: {
				// prueba -> Main.combinar
				uint32_t __cookie;
				int combinar_izquierda;
				readfull(&__prueba_out, &combinar_izquierda, sizeof(int), 1);
				int combinar_mitad;
				readfull(&__prueba_out, &combinar_mitad, sizeof(int), 1);
				int combinar_derecha;
				readfull(&__prueba_out, &combinar_derecha, sizeof(int), 1);
				readfull(&__prueba_out, &__cookie, sizeof(__cookie), 1);

				combinar(combinar_izquierda, combinar_mitad, combinar_derecha);

				writefull(&__prueba_in, &__msgid, sizeof(__msgid));
				writefull(&__prueba_in, &__cookie, sizeof(__cookie));
				writeflush(&__prueba_in);
				break;
			}
...
void mezclar(int izquierda, int derecha) {
	...
	writefull(&__prueba_in, &__msgid, sizeof(__msgid));
	writefull(&__prueba_in, &izquierda, sizeof(int));
	writefull(&__prueba_in, &derecha, sizeof(int));
	writefull(&__prueba_in, &__cookie, sizeof(__cookie));
	writeflush(&__prueba_in);

	__message_loop_prueba(__msgid,  0 );
	uint32_t __cookie_result = 0;
	readfull(&__prueba_out, &__cookie_result, sizeof(__cookie_result), 1);
