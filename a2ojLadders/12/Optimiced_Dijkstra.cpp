...
void dijkstra(int start, int end, vector < vector < node > > graph)
{
...
int main()
{
	...
	int E; // num of edjes;
	cin>>V>>E;
	vector < vector < node > > graph(V+1); // reseve memory
	...
	for(int c = 0 ; c<V; c++)
	{
		...
		int A,B,X;
		cin>>A>>B>>X;
		graph[A].push_back({B,X,0});
		/// other form of make push in pair struct 
		/// graph[A].push_back( {.v = B, .weight = X, .total = 0});
		graph[B].push_back({A,X,0});
	}
