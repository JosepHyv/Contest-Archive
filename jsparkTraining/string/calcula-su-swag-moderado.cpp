...
int main()
{
 	...
 	{
 		// a e i o u
 		if(names[c] == 'a' || names[c] == 'e' || names[c] == 'i' || names[c] == 'o' || names[c] == 'u')
 		{
 			vocales ++;
 		}
 	...
 	string apodo = "";
 	for(int c = names.size() -1; c >= 0 && names[c] != ' '; c--)
 		apodo += names[c];
 	cout<<apodo;

 	// no se preocupen no ando hablando 
 	// tengo mi teclado en ingles y no se usarlo xd
 	// :p
